/**
 * Локальный (офлайн) аккаунт — режим по умолчанию.
 *
 * Игра стартует сразу, без входа: Supabase не вызывается, прогресс, сложность
 * и промокоды живут в localStorage (кэше браузера). Все игровые функции при
 * этом остаются доступными. Вход в облачный аккаунт выполняется через
 * настройки; пара `wolf` / `123456` остаётся зарезервированным локальным
 * профилем — с ней всегда действует локальный режим.
 */
...
/** Локальный пароль, заданный игроком в настройках (базовый `123456` работает всегда). */
